...
agent.run(
    "Use the hpo_lookup tool to map the following clinical string to the correct HPO terms but please do not map any negations only positively asserted terms and do not include any non-clinical terms. Please first extract the clinically meaningful items and then use the tool as provided to extract terms, don't make random guesses and try to avoid thinking repetition and hallucinations: "
